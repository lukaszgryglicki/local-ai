...
- DONE: hardware audit; model research incl. two wide sweeps (`research/`);
  ranked shortlist by tier (§4: T0 North-Mini-Code, T1 Qwen3.6-35B-A3B, T1b
  KAT-Coder, T2, T3 Qwen3.8-Flash-Next); configs (§5); llama.cpp v0.4.0
...
- NEXT when unblocked: §7 step 4 onwards (North UD-IQ3_XXS download + smoke
  + 256K warm-up, 9B Q8_0 head-to-head, then T1/T1b, T2 grid, T3 on a master
  build).
...
| power | S3 works (`z` = `sudo zzz`); lid = display off/on only | **a server with GPU work in flight hangs forever after a resume** (fence wait, live test 2026-09-12 07:26, results-t0.md) → stop llama-server before your own `z`; the thermal watchdog's suspend does it automatically and restarts after (§6 rule 5, `ops.md`) |
...
1. Thinking model, coding-specialised, **≥ 256K context** (native preferred;
   ...
   native fits — currently moot, T1 fits natively).
...
4. **Tiers (Łukasz's final definition, 2026-09-11):**
   - **T0 — no compromises:** everything (weights, KV q8_0 @ 262 144,
     compute buffers) in the 16 GiB Quadro, native 256K, thinking, dev-focused;
     the largest quant that fits. Nothing in RAM.
   - **T1 — between T0 and T2:** as T0 but *either* a RAM-resident cold part
     costing ≤ 30 % tg, *or* 128K native + YaRN ×2 (only if nothing native
     fits — currently moot). Still thinking + dev-focused.
   - **T2:** RAM-resident experts at ≤ 1.5–3× slower than the all-VRAM layout.
   - **T3:** the bigger-model experiment — experts in the 128 GiB, quality first.
     **Status (owner, 12 Sep 11:23): kept as an option, decided at the very end** —
     T2 is already at the edge of usability, so T3 only happens if T2 leaves room.
...
|---|---|---|---|---|---|---|---|---|---|
| **T0** | **North-Mini-Code-1.0** (CohereLabs, 2026-06-05; `cohere2moe`: 49 layers = 1 dense + 48 MoE, 128 experts top-8 + shared, 13 full-attn (NoPE) + 36 SWA-4096, kv 4 × 128, vocab 262 144, tied embeddings, no MTP) — `unsloth/North-Mini-Code-1.0-GGUF` (imatrix UD; bartowski too) | **UD-IQ3_XXS 10.89 GiB** (experts 9.71 = 0.202 GiB per MoE layer, IQ3_XXS/IQ2_S; dense 0.77; token_embd Q6_K 0.41) · UD-Q2_K_XL 9.76 / UD-IQ2_M 9.19 (pure T0 with margin) · UD-IQ3_S 11.89 (k≈4–5 → T1) · UD-IQ4_XS 14.18 (k≈16 → T2 ≈ 1.8–2×) · UD-Q4_K_XL 17.93 (k≈34) · Q8_0 30.21 (> 3× ✗) | yes — interleaved reasoning; the fork has the Cohere2-MoE/"North Code" chat parser (`common/chat.cpp:2553`) | **500 000** (`rope_scaling: none`, θ 50 000) | weights 10.89 (incl. the 0.41 GiB `output` copy) + KV q8 **3.45** (13 full layers × 4 × 128) + SWA cache 0.17 + compute ~1.2 ≈ **15.7 GiB ≈ 98 % of 16.0** — exactly the wanted tight fit, zero margin: if it OOMs, `--n-cpu-moe 1` (15.5) or `2` (15.3) costs ≈ −6 / −12 % (a CPU MoE layer reads 8/128 × 0.202 GiB = 12.6 MiB/token ≈ 0.6 ms incl. sync) and is still T1-class; UD-Q2_K_XL → 14.6 GiB (91 %) is the pure-T0 fallback | ~0.5 GiB (token_embd) | ~1.8 GiB read/token (0.77 dense + 0.41 output + 0.61 experts) → **~100–130 t/s** all-GPU, 3–4× the dense 9B; no MTP; pp fast (3B active) | **SWE-V 67.6, LCB-v6 70.3, TB2 36.0** (TB-Hard 31.1, SWE-Pro 40.2; Cohere's chart, transcribed; bf16) — between Qwen3.5-9B (LCB 65.6, no SWE-V) and Qwen3.6-35B-A3B (73.4 / 80.4 / 51.5) | **the only coding-specialised thinking model that fits entirely in 16 GiB at 262K**, and the fastest. Risks: 3-bit experts (loss unmeasured — compare with the 9B Q8_0 on real tasks), arch only 3 months old in llama.cpp, benchmarks come from an image, 98 % fill may OOM on compute buffers (then k=1–2) |
| T0 safe | **Qwen3.5-9B** (dense hybrid GDN 24+8 full-attn, 4 KV × 256) — `unsloth/Qwen3.5-9B-MTP-GGUF` (no 3.6/3.8-9B exists) | **Q8_0 9.11 GiB** (default — largest that fits; UD-Q8_K_XL 12.34 does not) · UD-Q6_K_XL 8.37 only if Q8 OOMs on compute buffers | yes (default; `enable_thinking:false` to disable) | 262 144 | **everything on the Quadro**: weights 9.11 + KV q8 **4.25** + GDN state 0.05 + compute ~1.2 ≈ **14.6 GiB ≈ 94 % of VRAM** — the wanted tight fit | ~1 GiB (token_embd stays on CPU by default) | **30–35 t/s** raw (dense, memory-bound on 448 GB/s); ×1.6–1.9 with `draft-mtp` → 50–65 t/s; pp ≈ 1 k t/s short, GDN-prefill caveat at 100K+ | LCB-v6 **65.6** (= Qwen3-30B-A3B-Thinking-2507 66.0; T1/T2 80.4, gpt-oss-20b 74.6), BFCL-V4 66.1, TAU2 79.1, LongBench-v2 55.2; **no SWE-V on the card** | the *safe* T0: dense Q8_0 (no quant risk), MTP head, 6 % margin; 3–4× slower than North and weaker on paper (LCB 65.6 vs 70.3, no SWE-V) — the reference North must beat on real tasks. Same `qwen35` arch/rules as T1/T2 (never `--no-kv-offload`, never `-b 512`). Fill the last ~0.9 GiB by raising `-ub` for pp, not by KV f16 (K f16 alone = +4 GiB) |
| T0 alt | **Gemma-4-26B-A4B-it** (Google, 2026-03-11; `gemma4`: 30 layers = 5 full (kv 2 × 512, K==V stored twice) + 25 SWA-1024 (kv 8 × 256), 128 experts top-8 + shared MLP, tied embeddings, MTP drafter) — `unsloth/gemma-4-26B-A4B-it-GGUF` | UD-IQ3_S 10.63 / UD-IQ3_XXS 10.51 (all-VRAM 14.7 / 14.5 GiB incl. the 0.56 GiB output copy) · UD-Q3_K_M 11.84 → 15.9 (k=1–2) · UD-IQ4_XS 12.65 → k≈4 (T1) · UD-Q4_K_XL 15.83 → k≈10 (T2 ≈ 1.9×) · Q8_0 25.0 (> 3× ✗). `moe_intermediate 704` (not ×256) → expert down-proj falls back to IQ4_NL/Q5_1 in every i-quant (good for quality, sets the size floor) | `<|think|>` via chat template | 262 144 | KV q8 2.66 + SWA 0.16 + compute 1.2 | ~0.6 | ~100–115 t/s (4B active) | **LCB-v6 77.1**, Tau2 68.2 (Google card); SWE-V 57.4 / TB2.1 37.2 (NVIDIA's harness) | best *generalist* that fits all-VRAM: beats North on LCB, clearly loses on SWE-V; not coding-specialised → second choice. **Run without the MTP drafter on Vulkan** (#24664/#28188/#27460 crashes) |
| T0b | Qwen3.6-35B-A3B fully in VRAM (test-only) — MTP repo `unsloth/Qwen3.6-35B-A3B-MTP-GGUF` or non-MTP `unsloth/Qwen3.6-35B-A3B-GGUF`; coding fine-tune `bartowski/Kwaipilot_KAT-Coder-V2.5-Dev-GGUF` (no MTP, imatrix) | **UD-IQ2_M 11.07 (MTP) / 10.73 (non-MTP)**, UD-Q2_K_XL 11.45 (non-MTP), KAT IQ2_M 11.24 — all 2-bit experts | yes | 262 144 | + KV 2.66 + state 0.06 + compute 1.2 → **15.0 / 14.65 / 15.37 / 15.16 GiB (97 / 94.5 / 99 / 98 %)** — OOM risk on compute buffers for the 99 % one | ~1 GiB | 60–130 t/s raw all-GPU (bandwidth model says ~130, Turing-Vulkan reality lower); MTP file ×1.6–1.9 on top | T2 minus an unknown 2-bit loss (KAT: SWE-V 69.4 on its harness, see T1 row) | the only way to get a 35B-A3B *entirely* into 16 GiB; measure quality against T1 once, keep only if the 2-bit loss is invisible on real tasks (unlikely) |
| **T1** | **Qwen3.6-35B-A3B "cold experts in RAM ≤ 30 %"** — non-MTP `unsloth/Qwen3.6-35B-A3B-GGUF` for the tightest fit, MTP repo when the extra layers still fit the rule | **non-MTP UD-IQ3_S 12.74 GiB** (≈ 0.28 GiB/expert-layer) · non-MTP UD-IQ3_XXS 12.30 (0.256/L) · MTP UD-IQ3_XXS 13.10 (0.27/L; the MTP head costs 0.3–1.6 GiB depending on quant: IQ3_S 14.29 vs 12.74!) | yes | 262 144 | budget for experts ≈ 15.5 − 1.2 − 2.66 − 0.06 − non-expert weights (1.5–1.9) ≈ **9.7–10 GiB** → non-MTP IQ3_S: 35 of 40 layers in VRAM → **`--n-cpu-moe 5`**; non-MTP IQ3_XXS: **`--n-cpu-moe 3`**; MTP IQ3_XXS: **`--n-cpu-moe 7`**. Start one layer higher than the arithmetic, lower until OOM (fill VRAM) | 1–2.2 GiB | per token the CPU reads k × (GiB/L)/32 (+ k host syncs ≈ 0.2 ms each): k=3 ≈ +1.4 ms (**−10–15 %**), k=5 ≈ +2.4 ms (**−15–25 %**), k=7 ≈ +3.5 ms (−25–45 %, depends on the all-GPU base: 7.5–11 ms/token) → 50–110 t/s raw; MTP file adds ×1.6–1.9 on thinking/code output | as T2 minus the 3-bit loss (Qwen card: SWE-V 73.4, LCB-v6 80.4, TB2 51.5; KAT's harness puts the base at SWE-V 64.4) | MoE experts are *per-token sparse* (8 of 256): k CPU layers cost ≈ k/32 of their bytes per token, so ≤ 8 layers ≈ ≤ 30 %, 20 layers ≈ 2× (config A), 40 ≈ 3–4× (config B). Pick: **non-MTP UD-IQ3_S `--n-cpu-moe 5`** (best quant that respects the rule with margin) vs **MTP UD-IQ3_XXS `--n-cpu-moe 7`** (rule edge, but ×1.6–1.9 from draft-mtp on the long thinking traces where ngram does not help) — measure both |
| T1b | **KAT-Coder-V2.5-Dev** (Kwaipilot, 2026-07-23) = Qwen3.6-35B-A3B **fine-tuned for agentic coding**, thinking kept (`enable_thinking`, `preserve_thinking`), no MTP head — `bartowski/Kwaipilot_KAT-Coder-V2.5-Dev-GGUF` (imatrix, b10087) | IQ3_XXS 13.85 (0.31/L) → `--n-cpu-moe 8` · IQ3_M 15.74 → ~14 · IQ4_XS 17.51 → ~16 · Q4_K_M 19.92 → ~20 · Q6_K 27.99 → ~27 | yes | 262 144 | same geometry as T1 (KV 2.66) | 2.5–20 GiB | IQ3_XXS k=8 ≈ −30–45 % (T1 edge); Q4_K_M k=20 ≈ 2× and Q6_K k=27 ≈ 2.9× (T2) | **SWE-V 69.4, SWE-Multilingual 63.0, SWE-Pro 46.0, TB-2.1 41.0** — on KAT's own harness where the Qwen3.6-35B-A3B base scores 64.4 SWE-V, i.e. **+5 over base** | the only *coding-specialised* thinking model in the 35B-A3B class — the literal reading of the requirement. Costs: no MTP (−40 % effective tg vs the MTP Qwen file), bartowski IQ quants instead of unsloth UD. Run it head-to-head with T1 on 3–5 real repo tasks before choosing |
| — | gpt-oss-20b (`gpt-oss`, 24 layers = 12 full + 12 SWA-128, 32 experts top-4, attention sinks) — `ggml-org/gpt-oss-20b-GGUF` | MXFP4 **11.28 GiB** (= 12.1 GB — the earlier "16.6 GiB" mixed GB and GiB) | `reasoning_effort` low/med/high | **131 072** = YaRN ×32 from 4 096 already; 262K needs `--rope-scale 64 --yarn-orig-ctx 4096` | 12 full-attn × 8 KV × 64: KV q8 **3.19** @262K + weights 10.71 (untied, token_embd on CPU) + compute 1.2 ≈ **15.1 GiB — fits (94 %)** | 0.6 | ~90 t/s | LCB-v6 74.6 / SWE-V 60.7 (Qwen's table; 52.4 on NVIDIA's), **TB2.1 15.2** | fits after all, but 256K only by doubling an already ×32 YaRN (quality unverified) and weak agentic (TB 15) → T1(i) fallback at best, dominated by North and T1 |
| **T2** | **Qwen3.6-35B-A3B** — `unsloth/Qwen3.6-35B-A3B-MTP-GGUF` | UD-Q4_K_XL 21.3 GiB (speed) · Q8_0 35.2 / UD-Q6_K_XL ~27.8 (quality) | yes (default; `preserve_thinking` opt-in) | 262 144 | rest 2.5 + KV 2.66 + state 0.06 ≈ 5.3 GiB → **~9 GiB spare = 18–20 of 40 expert layers @ Q4** (0.46 GiB/layer; Q8: 11 layers @ 0.77) | 9–18 GiB | 10–20 t/s (×1.5–2.5 with draft-mtp+ngram-mod) | SWE-V **73.4**, TB2 51.5, LCB-v6 80.4, TAU3 67.2 | best score/speed ratio of everything that fits; `qwen35moe` mature (b7990), MTP head embedded (b9180+, ~75 % acceptance, >2×). Vulkan: never `--no-kv-offload` (#24519 gibberish), never `-b 512` (#27237), watch decode collapse #26795 (AMD-only so far) |
| **T3** | **Qwen3.8-Flash-Next** (125B-A6B + 51B n-gram table + MTP) — `unsloth/Qwen3.8-Flash-Next-GGUF` | **UD-IQ4_XS 87.3 GiB** (UD-Q4_K_XL 103.7 is too tight for 128 GiB) | yes; `reasoning_effort` xhigh/medium/low, `preserve_thinking` default on | 262 144 | rest 5.1 + KV 3.19 + QSA indexer-K 0.75 + state 0.11 ≈ 9.2 GiB (experts all on CPU) | experts ~60 + n-gram table ~27 (host-side `get_rows`, stays in RAM) ≈ **87 GiB** → ARC cap ≤ 8 GiB, no dockaws VM | 4–7 t/s (+MTP only with unsloth fork / PR #28243 — the remote node already runs a patched build) | SWE-Pro **62.5**, DeepSWE 58.7, LCB-v6 **91.9**, Toolathlon 73.5 | the quality stretch; the model already proven on the remote CPU node (`remote/serve-new.sh`). Needs llama.cpp **≥ b10889** (master; v0.4.0 has qwen4exp base but not #27880/#27941/#28023/#28123/#28330). Vulkan sparse-FA for QSA still open (#28105); open Vulkan bugs #27431/#28158/#28280 |
| T4 | Qwen3.5-122B-A10B — `unsloth/Qwen3.5-122B-A10B-MTP-GGUF` | UD-Q4_K_XL 73.3 GiB (UD-IQ4_XS 57.7) | yes | 262 144 | rest 6.2 + KV 3.19 + 0.15 ≈ 9.6 GiB | 65 GiB | 3–5 t/s | SWE-V 72.0, TB2 49.4, LCB 78.9 | **dominated by T2 on paper** (lower scores, 3× slower); only if T2 quality disappoints and T3 is unusable on Vulkan |
| T5 | Nemotron-3-Super-120B-A12B (`NEMOTRON_H_MOE`, MTP) | Q4 ≈ 59 GiB experts | yes | 256K (8 attn layers → KV q8 @256K only 1.06 GiB) | ~6 GiB | ~60 GiB | 3–5 t/s | SWE-V ≈ 60 | non-Qwen fallback family if Qwen hybrids misbehave on NVIDIA Vulkan; Mamba-SSM Vulkan perf concern #23348 |
| T6 | Step-3.7-Flash 196B-A11B | Q4/IQ4_XS ≈ 100–106 GiB experts | yes | 128K native, 256K via RoPE ×2 | ~9 GiB (KV 6.4) | >100 GiB | 3–5 t/s | SWE-V 74.4, TB2.1 59.5 | does not fit 128 GiB at Q4 with headroom; 256K not native → conditional, unlikely |
...
< T1/T2 *and* no fully-in-VRAM file exists: `moe_intermediate 1856` is not a
...
offload it would only be a worse T1), Ling-3.0-flash (tool-call parser PR
#28682 still open).
...
TB2.0 21.1 and the same 16.9 GiB k-quant expert floor as Lightning → T2 only at
...
no SWE/TB, ~25 t/s — only if both T0 picks disappoint), Gemma-4-E2B/E4B,
...
LCB 55.8 / SWE-V 33.6 — a weaker T0), Ling-3.0-tiny (7.9B-A1.3B KDA+MLA,
131K + YaRN, Q8_0 7.8 GiB, TB2.1 27.7, arch `bailingmoe3` is in the fork, but
MLA long-context FA bug #28124), GLM-4.7-Flash (202K native, MLA bug, T2 2×),
...
(T0) or Qwen3.6-35B-A3B / KAT-Coder (T1/T2) on the coding + thinking + 256K +
fits intersection.
...
|---|---|---|---|---|---|
| **T0** | **North-Mini-Code-1.0 UD-IQ3_XXS** (`--n-cpu-moe 0`; 1–2 only if it OOMs) | 10.9 GiB | **98 % VRAM (15.7 GiB)**, zero margin | 100–130 t/s | coding-specialised, thinking, 500K native, all-GPU, fastest — verify the 3-bit quality vs the 9B |
| T0 safe | Qwen3.5-9B **Q8_0** | 9.1 GiB | 94 %, nothing in RAM | 30–35 t/s (50–65 with MTP) | no quant risk, MTP; slower, LCB 65.6, no SWE-V — the reference |
| T0 alt | Gemma-4-26B-A4B-it **UD-IQ3_S** (no MTP drafter) | 10.6 GiB | 92 % (14.7 GiB) | 100–115 t/s | generalist: LCB 77.1 but SWE-V 57 — if North's quality disappoints |
| T0b | Qwen3.6-35B-A3B **UD-IQ2_M** (non-MTP 10.73) or KAT-Coder **IQ2_M** 11.24 | 10.7–11.2 GiB | 94–98 % | 60–130 t/s | 2-bit experts: quality unknown, test once |
| **T1** | Qwen3.6-35B-A3B **non-MTP UD-IQ3_S** `--n-cpu-moe 5` (alt: **MTP UD-IQ3_XXS** `--n-cpu-moe 7`) | 12.7 / 13.1 GiB | 15.3–15.5 GiB VRAM + 1.4–1.9 GiB RAM | 60–110 t/s (−15–25 % / −25–45 %; the MTP file wins ×1.6–1.9 back) | **best model within the ≤ 30 % rule**: SWE-V 73.4 / LCB 80.4 class, +6 SWE-V / +10 LCB over North |
| T1b | **KAT-Coder-V2.5-Dev IQ3_XXS** `--n-cpu-moe 8` | 13.9 GiB | 15.4 + 2.5 | −30–45 % | coding fine-tune of T1 (+5 SWE-V on KAT's harness), no MTP — head-to-head with T1 |
| **T2** | Qwen3.6-35B-A3B **UD-Q4_K_XL** `--n-cpu-moe 20`, then **Q8_0** `--n-cpu-moe 29` | 21.3 / 35.2 GiB | ~15.5 VRAM + 9–18 RAM | 2× / 3× slower than all-GPU | the 1.5–3× rule; Q8 = quality reference |
| **T3** | Qwen3.8-Flash-Next **UD-IQ4_XS** | 87.3 GiB | 9 GiB VRAM + 87 GiB RAM, ARC ≤ 8 GiB, no VM | 4–7 t/s | the only candidate clearly *better* than T2; master build ≥ b10889 |

Order of work: **T0 first — North-Mini-Code UD-IQ3_XXS (10.9 GiB)**: proves the
...
**T1** (+ T1b KAT-Coder, same class), then **T2** Q4 → Q8_0, then T3 on a master
build. Skip T0 alt / T0b / T4–T6 unless the ones above fail.

What each hard requirement is worth in 16 GiB: *coding-specialised* is now
satisfied literally (North in T0, KAT-Coder in T1b) — the generalists
(Gemma-4-26B, Cascade-2) only win on LCB; dropping *fully-in-VRAM* (≤ 30 %
RAM) → T1 (SWE-V 73 class) is still by far the best trade; dropping *256K →
128K native* buys only gpt-oss-20b (fits at 15.1 GiB, LCB 74.6 but TB 15),
Trinity-Mini and Granite-4.2-3b — none better than T1; dropping *thinking*
...
- **T0 (North UD-IQ3_XXS):** common set, `--n-cpu-moe 0`; on OOM at
...
- **T0 safe (Qwen3.5-9B Q8_0):** common set incl. `draft-mtp`, plus
  `--cache-ram 0` (§6 rule 2).
- **T1 / T1b:** common set with `--n-cpu-moe 5` (non-MTP UD-IQ3_S) / `7`
  (MTP UD-IQ3_XXS) / `8` (KAT IQ3_XXS); lower until OOM, then +1.

- **A — max-VRAM reference (T2):** `--n-cpu-moe 20` (layers 0–19 experts on
  CPU, 20–39 on GPU, Q4). Lower N until OOM; Q8: start `--n-cpu-moe 29`. This
  is the "VRAM-only-ish" baseline for the ≤1.5–3× rule.
- **B — all experts in RAM (T2 first, then T3/T4):** `--cpu-moe` (≡
...
For T3 a **master** build is required: `git -C .../llama.cpp fetch --tags`,
...
2. `timeout 900 build-vulkan/bin/test-backend-ops -b Vulkan0` — the gate for
   ...
   whole plan degrades to CPU-only (then T2 Q4 ≈ 8–14 t/s is the ceiling).
...
3. Dataset — **done 2026-09-11**: `zroot/data/local-ai` (compression=off,
   ...
   `/data/local-ai/models/`. Cap ARC for T3:
   `sysctl vfs.zfs.arc.max=$((8<<30))`.
4. Downloads (Wi-Fi only, ~10 MB/s; `sha256` each): **T0**
   ...
   `--n-cpu-moe 1–2`. Then **T0 safe** `unsloth/Qwen3.5-9B-MTP-GGUF`
   `Q8_0` (9.1 GiB ≈ 15 min, `draft-mtp`, `--cache-ram 0`) and the 3–5 task
   head-to-head. Then **T1** `unsloth/Qwen3.6-35B-A3B-GGUF/UD-IQ3_S`
   (12.7 GiB) — alt `…-MTP-GGUF/UD-IQ3_XXS` (13.1) — and **T2**
...
5. Grid (each ≥ 3 runs, server `/metrics` + `--verbose` timings): pp at
   ...
   → A/B at Q8_0.
6. T3 (optional — owner decides at the end, see §1): master build → `UD-IQ4_XS` (87 GiB ≈ 2.5 h at 10 MB/s) → B only
...
- **02:09 hard freeze** (2026-09-12, GPU 89 % + turbo 4.7 GHz, new request at
  ...
  BIOS event log are the owner's options (results-t0.md).
...
- **RAM contention**: T3 (87 GiB) + ARC + X leaves no room for the dockaws VM.
- **Model freshness**: Qwen3.8-Flash-Next arch landed 2026-08-27; expect
  breakage and re-builds; keep T2 as the stable daily driver.
...
**Tier numbering (owner, 13 Sep 2026 07:05): T0 = `fastest-vram`, T1 = `fast`, T2 = `best`, T3 = the optional bigger-model
tier.** Commits before `Update status 9` used T-1/T0/T1/T2 for the same four tiers (and `results-t1.md`/`report-t1.md` for
the T0 phase, `results-t0.md` for the T1 phase — the file names were swapped with the renumbering). Every T-number in this
repo, including the §4/§7 candidate rows, moved up by one on that date. The profile names never changed.
...
|---|---|---|---|
| `fastest-vram` | T0 | everything in the Quadro (weights, 256K q8_0 KV, compute); the fastest model that passes the coding tasks | expect **> 12 t/s**, never **< 10**, ideal **15–25 t/s** |
|  | **T0 result (12 Sep 21:46)** | **`qwen35b`** = Qwen3.6-35B-A3B UD-IQ2_M, `SPEC=none`, NP=1, ctx 262 144, `--cache-ram 8192`, `build-vulkan-2` — 2/4 E2E + a near-miss (North 0/4, qwen9b 1/4, gemma 1/4), frozen as the default of `start.sh`/`serve.sh`/`qwen.sh`/`llamactl.sh`; `results-t0.md` §5. **Frozen for good 13 Sep 00:30**: only `qwen35b` remains — North / qwen9b / gemma GGUFs deleted (32.8 GB) and their `models.sh` entries removed (git history keeps them) | measured **34.8–45.8 t/s** session aggregate, worst request 18.1 t/s at 230K — goal exceeded |
| `fast` | T1 | VRAM first, overflow to the **iGPU (Vulkan1) before CPU RAM** — whichever measures faster; no OOM, no "slow-token" regime | not slower than **6 t/s**, absolute minimum **4**, ideal **≥ 7–10 t/s** |
| `best` | T2 (T3 optional — owner decides at the end; T2 is already at the edge of usability) | as much as fits across VRAM + iGPU + RAM, whichever split is fastest, for the best quality | expect **≥ 1.8 t/s**, absolute low **1 t/s** (below = unusable), ideal **> 3 t/s** |
...
Order of work: finish T0 (this file + `results-t0.md`) → STOP → T1 → T2 → T3 only if the owner still wants it at the end; the
