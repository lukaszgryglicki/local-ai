#!/bin/bash
# /data/ai/bench2.sh - confirmation pass + small-model CPU-bound probe (test-db-02)
set -u
D=/data/ai
PORT=18081
BIGMODEL=$D/models/Qwen3.8-Flash-Next-UD-Q6_K_XL-00001-of-00006.gguf
RES=$D/bench2-results.txt

PROMPT_FILE=/tmp/bench-prompt.txt
[ -s $PROMPT_FILE ] || { echo "missing $PROMPT_FILE - run bench.sh once first"; exit 1; }
PROMPT=$(python3 -c "import json,sys; print(json.dumps(open('$PROMPT_FILE').read()))")

run_variant() {
  local name="$1"; local model="$2"; shift 2
  echo "=== variant: $name ($*)" | tee -a $RES
  systemd-run --scope --collect -q -p MemoryHigh=190G -p MemoryMax=200G -p AllowedCPUs=0-43 \
    $D/llama-server --model "$model" --host 127.0.0.1 --port $PORT \
    --jinja --temp 1.0 --top-p 0.95 --top-k 20 --min-p 0.0 \
    --no-mmproj --no-ui --no-agent --offline --api-key-file $D/key.secret \
    --log-file /tmp/bench-$name.log "$@" &
  local SPID=$!
  for i in $(seq 1 150); do
    curl -s -o /dev/null -w "%{http_code}" http://127.0.0.1:$PORT/health 2>/dev/null | grep -q 200 && break
    sleep 2
  done
  KEY=$(cat $D/key.secret)
  for r in 1 2 3 4; do
    curl -s -H "Authorization: Bearer $KEY" -H "Content-Type: application/json" \
      -d "{\"prompt\": $PROMPT, \"n_predict\": 256, \"temperature\": 0, \"cache_prompt\": true}" \
      http://127.0.0.1:$PORT/completion > /dev/null
  done
  sleep 1
  grep "prompt eval time" /tmp/bench-$name.log | awk -F'[/(,]' \
    '{split($2,a," "); split($4,s," "); n++; if(a[1]>1000){pp[n]=s[1]}} END {printf "  cold-pp tok/s:"; for(i in pp) printf " %s", pp[i]; print ""}' | tee -a $RES
  grep -E "\| *eval time" /tmp/bench-$name.log | awk -F'[(,]' \
    '{split($3,s," "); v=v" "s[1]} END {print "  tg tok/s:" v}' | tee -a $RES
  kill $SPID 2>/dev/null; sleep 3
  fuser -k $PORT/tcp 2>/dev/null; sleep 2
}

# 1) reference: exact current production shape
run_variant prod-now "$BIGMODEL" --ctx-size 1048576 --parallel 4 \
  --cache-type-k q8_0 --cache-type-v q8_0 --batch-size 2048 --ubatch-size 1024 \
  --threads 24 --threads-batch 44 --spec-type ngram-simple --reasoning-effort low

# 2) proposed: t16 + ngram-mod + cache-ram -1 (same 4x256K shape)
run_variant prod-new "$BIGMODEL" --ctx-size 1048576 --parallel 4 \
  --cache-type-k q8_0 --cache-type-v q8_0 --batch-size 2048 --ubatch-size 1024 \
  --threads 16 --threads-batch 44 --spec-type ngram-mod --cache-ram -1 --reasoning-effort low

# 3) repeat proposed to check ngram-mod variance
run_variant prod-new2 "$BIGMODEL" --ctx-size 1048576 --parallel 4 \
  --cache-type-k q8_0 --cache-type-v q8_0 --batch-size 2048 --ubatch-size 1024 \
  --threads 16 --threads-batch 44 --spec-type ngram-mod --cache-ram -1 --reasoning-effort low

# 4) small dense 7B Q4_K_M (4.7GB): CPU-bound probe
run_variant small-7b $D/models-small/qwen25-coder-7b-q4km.gguf \
  --ctx-size 32768 --parallel 1 --batch-size 2048 --ubatch-size 1024 \
  --threads 16 --threads-batch 44 --spec-type none

# 5) tiny dense 1.5B Q8 (1.6GB): bandwidth-scaling probe
run_variant small-1.5b $D/models-small/qwen25-coder-1.5b-q8.gguf \
  --ctx-size 32768 --parallel 1 --batch-size 2048 --ubatch-size 1024 \
  --threads 16 --threads-batch 44 --spec-type none

echo "bench2 done - see $RES"
