#!/bin/bash
# /data/ai/bench3.sh - MTP draft-head + tuned ngram-mod benchmarks (patched build)
set -u
D=/data/ai
PORT=18081
...
# 0) control on patched binary: plain ngram-mod (checks binary parity + ngmod re-run)
run_variant ngmod-ctl  --spec-type ngram-mod

# 1) MTP with Q4 head, author-recommended knobs
run_variant mtp-q4     --model-draft $MTPQ4 --spec-type draft-mtp --spec-draft-n-max 3 --spec-draft-p-min 0.75

# 2) MTP with BF16 head
run_variant mtp-bf16   --model-draft $MTPBF --spec-type draft-mtp --spec-draft-n-max 3 --spec-draft-p-min 0.75

# 3) MTP + ngram-mod combined
run_variant mtp-ngmod  --model-draft $MTPQ4 --spec-type draft-mtp,ngram-mod --spec-draft-n-max 3 --spec-draft-p-min 0.75

# 4) tuned long-draft ngram-mod (docs: MoEs require long drafts)
run_variant ngmod-long --spec-type ngram-mod --spec-ngram-mod-n-match 24 --spec-ngram-mod-n-min 48 --spec-ngram-mod-n-max 64

echo "bench3 done - see $RES"
