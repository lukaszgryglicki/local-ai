...
exec "$d/llama" --model "$d/model.gguf" --alias qwen3coder-local --host 10.253.254.1 --port 18080 --ctx-size 262144 --parallel 1 --gpu-layers 99 --cpu-moe --no-kv-offload --load-mode none --lazy-mode off --cache-type-k q8_0 --cache-type-v q8_0 --flash-attn on --batch-size 2048 --ubatch-size 1024 --threads 12 --threads-batch 12 --spec-type ngram-simple --jinja --reasoning-effort medium --temp 1.0 --top-p 0.95 --top-k 20 --min-p 0.0 --repeat-penalty 1.0 --no-mmproj --no-ui --no-agent --offline --timeout 43200 --api-key-file "$d/key.secret" --log-file /tmp/local-ai-llama.log
